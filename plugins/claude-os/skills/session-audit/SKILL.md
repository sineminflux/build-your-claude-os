---
name: session-audit
description: End-of-session audit for your Claude OS. Scans the conversation for corrections, preferences, decisions, and new facts that should be saved, and proposes exactly where each goes across your CLAUDE.md and MEMORY.md files. If you connected a task system during setup, it also catches tasks you mentioned that are not tracked yet and, on your confirmation, creates them. Also runs an on-demand folder-health pass that prunes bloat and checks the OS stays lean and consistent. Everything is proposed first and written only on your approval. Use when you say "audit this session", "session audit", "what did we miss", "end of session check", "save what we learned", or for the health pass "tidy my OS", "OS health check", "check my OS for bloat", "audit my OS folders".
---
...
- **A correction about writing or voice?** (a banned word, a phrasing to avoid, a tone rule) goes in the never-list inside `00_Shelf/voice-guidelines.md`, so the `no-ai-slop` skill picks it up on the next write. This is how the OS learns writing corrections; it never edits the shipped skills.
...
Prune as you write, so files do not grow without end:
- A fact that updates an existing one gets edited in place, never stacked as a new line under the stale one ("moved the launch to March" replaces the old date, it does not sit beneath it).
- Skip anything already saved (you filtered in Step 4); do not restate it in different words.
- If the file you are writing to is over its size budget (see the Front Desk's Leanness section), compact it first: merge duplicate rules, cut superseded facts, replace a repeated detail with a one-line pointer. Propose the compaction, then write on approval.

Never write anything (file or task) that was not approved. Present first, wait.

## Folder health (on demand)

Run this pass when the reader says "tidy my OS", "OS health check", "check my OS for bloat", or "audit my OS folders". It is report-first: propose everything, change nothing until they approve, the same as the rest of this skill.

Read the OS (root `CLAUDE.md` and `MEMORY.md`, `00_Shelf/`, and each department's pair), then check for the things that let a workspace rot:

- **Over-budget files.** Any file past its cap in the Front Desk's Leanness section. Propose a compaction (merge duplicates, cut superseded facts, pointer instead of paragraph).
- **Duplicates and near-duplicates.** The same rule or fact written more than once across files, or restated in different words. Propose keeping one and cutting the rest.
- **Contradictions.** Two files disagreeing on a fact: two prices, two positioning lines, a decision reversed while the old one still sits there. Propose which is current and cut the other.
- **Stale facts.** A decision, status, or date that a later one supersedes but that still sits in a `MEMORY.md`. Propose editing it to the current truth.
- **Excess.** An entry long enough to be a pointer, or a desk that finished but was never moved to `_archive/`. Propose trimming or archiving.
- **Dead routes and orphans.** A Floor Plan row pointing at a department folder that no longer exists, a Shelf row pointing at a missing file, a department folder with no Floor Plan row, or a Shelf file no trigger points to. Propose adding the missing row or removing the dead one.
- **Broken links.** External links in `brand.md` that no longer resolve. Check them and propose fixing or removing.
- **Slop that crept in.** Saved copy that breaks the writing rules (em dashes, the "not X, it's Y" pivot, throat-clearing). Propose the rewrite.

Present the findings as a short menu grouped by type, each with the exact edit and a one-line why, marked "Your call". Apply only what they approve, dropping the marker on each write. If everything is healthy, say so in one line.

If they ran this by hand and there is no recurring tidy scheduled yet, suggest putting it on a schedule so they do not have to remember: "Want me to set this to run every quarter on its own?" On their yes, create the recurring scheduled task through Cowork's scheduling (report-only, opening this workspace in a fresh session each run). Create it only on their okay.
